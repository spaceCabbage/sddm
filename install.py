...
subprocess.run("sudo rm -rf /usr/share/sddm/themes/nsa/", shell=True)
subprocess.run("sudo mkdir /usr/share/sddm/themes/nsa/", shell=True)

extension = Path(wallpaper).suffix
content = content.replace(
    "background=",
    f"background=/usr/share/sddm/themes/nsa/wallpaper{extension}",
)
result = subprocess.run(
    f"sudo cp '{wallpaper}' /usr/share/sddm/themes/nsa/wallpaper{extension}",
    shell=True,
    check=True,
)
print(result.stdout)
...
subprocess.run(f"sudo cp -r {src_dir}/* /usr/share/sddm/themes/nsa/", shell=True)
